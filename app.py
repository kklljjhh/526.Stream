...
def process_excel(file_path):
    """
    处理单个 Excel 文件，返回输出文件路径或 None。
    注意：原函数使用了 progress_queue，这里移除；错误改为抛出异常。
    """
    ...
    if a1_value and str(a1_value).strip().endswith("实时数据"):
        ...
        with open(failed_txt, "w", encoding="utf-8") as f:
            f.write("文件是实时数据，已跳过处理。\n")
        return None   # 返回 None 表示跳过，但文件本身不生成
...
# ------------------------------------------------------------
# Web 部分：上传 ZIP → 解压 → 处理 → 打包下载
# ------------------------------------------------------------
HTML_TEMPLATE = '''
<!doctype html>
<title>Excel 批量处理工具</title>
<h2>上传 ZIP 压缩包（内含 .xls/.xlsx 文件）</h2>
...
def index():
    if request.method == 'POST':
        ...
        if not f or f.filename == '':
            return render_template_string(HTML_TEMPLATE, error='请选择一个ZIP文件')

        if not f.filename.lower().endswith('.zip'):
            return render_template_string(HTML_TEMPLATE, error='仅支持ZIP格式')
        ...
        try:
            ...
            # 遍历解压后的文件夹（支持嵌套，这里只处理根目录和直接子文件夹）
            excel_files = []
            for root, dirs, files in os.walk(tmpdir):
                for file in files:
                    if file.lower().endswith(('.xls', '.xlsx')) and "_处理后" not in file:
                        excel_files.append(os.path.join(root, file))

            if not excel_files:
                return render_template_string(HTML_TEMPLATE, error='压缩包中没有找到 .xls 或 .xlsx 文件')

            # 收集所有生成的文件路径
            output_files = []
            for fp in excel_files:
                try:
                    out = process_excel(fp)
                    if out:   # 只有处理成功且不跳过的文件才添加
                        output_files.append(out)
                    # 注意：如果是“实时数据”跳过的文件，不会产生 _处理后.xlsx，但可能有失败提示文件
                except Exception as e:
                    # 出错也记录一个 txt 文件，方便用户知道
                    err_dir = os.path.join(os.path.dirname(fp), "处理后数据")
                    os.makedirs(err_dir, exist_ok=True)
                    err_file = os.path.join(err_dir, f"{os.path.splitext(os.path.basename(fp))[0]}_错误.txt")
                    with open(err_file, "w", encoding="utf-8") as ef:
                        ef.write(f"处理错误：{str(e)}\n")
                    output_files.append(err_file)

            if not output_files:
                return render_template_string(HTML_TEMPLATE, error='所有文件处理均失败或为实时数据，无结果可下载')

            # 打包所有输出文件到一个 ZIP
            mem_zip = BytesIO()
            with zipfile.ZipFile(mem_zip, 'w', zipfile.ZIP_DEFLATED) as zf:
                for ofp in output_files:
                    # 保留相对路径结构
                    arcname = os.path.relpath(ofp, tmpdir)
                    zf.write(ofp, arcname)
